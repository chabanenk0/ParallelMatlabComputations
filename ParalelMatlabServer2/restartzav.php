...
if ($id>=0) $r=mysql_query("select id,  command, state,done,IP, adduserid, predictminutes, datediff(curdate(),begcalcdate) as elapseddays, time_to_sec(timediff(curtime(),begcalctime)) as elapsedseconds from tasks having ((state='calc') and (adduserid=$id) and (elapseddays*24*60+elapsedseconds/60.0 > predictminutes))");
else
$r=mysql_query("select id, command, state,done,IP,predictminutes, datediff(curdate(),begcalcdate) as elapseddays, time_to_sec(timediff(curtime(),begcalctime)) as elapsedseconds from tasks having ((state='calc') and (elapseddays*24*60+elapsedseconds/60.0 > predictminutes))");
if (!$r)
 echo mysql_error();
$num_res=mysql_num_rows($r);
//echo '<h1>Следующие записи  будут перезапущены:';
